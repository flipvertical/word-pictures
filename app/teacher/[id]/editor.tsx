...
export default function Editor({ image, initialHotspots }: Props) {
  ...
  const analyze = () =>
    run("analyze", async () => {
      if (
        hotspots.length > 0 &&
        !confirm("Analyzing will replace all current hotspots and words. Continue?")
      ) {
        return "Analysis cancelled.";
      }
      const data = await api(`/api/teacher/analyze/${image.id}`, { method: "POST" });
  ...
  return (
    <main className="mx-auto max-w-5xl p-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {busy === "analyze" ? "Analyzing… (takes ~1 min)" : "Analyze with AI"}
        </button>
        <button
          onClick={save}
          disabled={busy !== null}
      ...
      <div className="mt-4 flex flex-col gap-6 lg:flex-row">
        <div className="lg:w-3/5">
          <div ref={imgWrapRef} className="relative inline-block w-full select-none">
            ...
            {selected?.box && (
              <div
                className="pointer-events-none absolute rounded border-2 border-white/80 mix-blend-difference"
                style={{
            ...
            {hotspots.map((h, i) => (
              <button
                ...
                title={h.label}
                className={`absolute flex h-7 w-7 -translate-x-1/2 -translate-y-1/2 cursor-grab items-center justify-center rounded-full text-xs font-medium text-white shadow ${
                  h.id === selectedId ? "bg-indigo-600 ring-2 ring-white" : "bg-neutral-900/80"
                }`}
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-xs text-neutral-500">
              Drag a numbered pin to reposition it. Click to select.
            </span>
            <button
              onClick={remove}
        ...
        <div className="lg:w-2/5">
          {!selected && (
            <p className="rounded-xl border border-dashed border-neutral-300 p-6 text-sm text-neutral-500">
              {hotspots.length === 0
                ? "No hotspots yet. Click “Analyze with AI” to propose some, or add one manually."
                : "Select a pin to edit its words."}
            </p>
          )}
          {selected && (
            <div className="rounded-xl border border-neutral-200 p-4">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg border border-neutral-300 px-3 py-1.5 font-medium"
                />
                <button
                ...
                <div key={pos} className="mt-4">
                  ...
                  <div className="mt-1 flex flex-col gap-1.5">
                    ...
                    {selected.words[pos].length === 0 && (
                      <p className="text-xs text-neutral-400">No {POS_LABELS[pos].toLowerCase()} yet.</p>
                    )}
                  </div>
                </div>
              ))}
